//! TUI task-start guardrails for concurrent runs and wide container scopes.
...
mod tests {
    ...
    fn blocks_wide_container_workspace() {
        let root = temp_root("dsx_tui_container_block");
    ...
    fn allows_project_root_workspace() {
        let root = temp_root("dsx_tui_project_root");
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn allows_explicit_wide_intent() {
        let root = temp_root("dsx_tui_wide_intent");
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join("1234")).unwrap();
    ...
    fn allows_markerless_static_project_root() {
        let root = temp_root("dsx_tui_static_project");
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn allows_narrowed_scope() {
        let root = temp_root("dsx_tui_narrowed");
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join("1234")).unwrap();
