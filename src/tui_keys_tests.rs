...
mod tests {
    use crate::tui_keys::{
        active_scope_path, handle_context_key, toggle_context, toggle_mission, toggle_settings,
        toggle_tools,
    };
    ...
    fn context_toggle_hides_other_modal_views() {
        ...
        assert!(!app.show_tools);
        assert!(!app.show_settings);
        assert!(!app.show_mission);
    }

    #[test]
    fn mission_toggle_hides_other_modal_views() {
        let mut app = dsx_tui::App::new();
        app.show_diff = true;
        app.show_tools = true;
