...
fn task_panel(app: &App) -> Paragraph<'static> {
    ...
    push_section(&mut lines, "Plan", &b.plan, Color::Green, 6);
    push_section(&mut lines, "Last", &b.last_changes, Color::LightBlue, 4);
...
fn ops_panel(app: &App) -> Paragraph<'static> {
    ...
    push_inline(&mut lines, "Reason", &app.scope_lock.reason, Color::Green);
    ...
    if app.tool_timeline.is_empty() {
        ...
        for entry in app.tool_timeline.iter().rev().take(8).rev() {
            ...
            lines.push(Line::from(vec![
                ...
                Span::raw(" "),
                Span::styled(entry.name.clone(), Style::default().fg(Color::LightBlue)),
            ]));
            lines.push(Line::from(vec![
                Span::raw("    "),
                Span::styled(entry.summary.clone(), Style::default().fg(Color::LightCyan)),
            ]));
        }
    }
    Paragraph::new(Text::from(lines))
        .block(block(" Scope / Tools ", Color::LightBlue))
        .wrap(Wrap { trim: false })
}
...
fn push_inline(lines: &mut Vec<Line<'static>>, title: &'static str, value: &str, color: Color) {
    lines.push(Line::from(vec![
        ...
        Span::styled(empty_as(value, "none"), Style::default().fg(Color::Green)),
    ]));
}
