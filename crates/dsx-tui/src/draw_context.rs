...
fn append_brief(brief: &TaskBriefPanel, lines: &mut Vec<Line<'static>>) {
    ...
    push_field(lines, "Last", &brief.last_changes, Color::Green, 3);
...
fn append_scope(app: &App, lines: &mut Vec<Line<'static>>) {
    push_inline(lines, "Launch", &app.scope_lock.launch_scope, Color::Green);
    push_inline(
        lines,
        "Active",
    ...
    if !app.scope_lock.warning.trim().is_empty() {
        push_inline(lines, "Check", &app.scope_lock.warning, Color::LightGreen);
    }
}
...
fn append_folder_notes(app: &App, lines: &mut Vec<Line<'static>>) {
    ...
    for (index, note) in app.folder_notes.iter().enumerate().skip(start).take(8) {
        ...
        lines.push(Line::from(vec![
            Span::raw("    next: "),
            Span::styled(note.next_step.clone(), Style::default().fg(Color::Green)),
        ]));
        lines.push(Line::from(vec![
            Span::raw("    arch: "),
            Span::styled(
...
fn append_focused_action(app: &App, lines: &mut Vec<Line<'static>>) {
    if let Some(scope) = app.focused_folder_scope() {
        push_inline(lines, "Focused path", &scope, Color::Green);
        push_inline(
            lines,
            "Focused action",
