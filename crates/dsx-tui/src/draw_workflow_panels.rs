...
impl App {
    pub(crate) fn draw_task_brief_panel(&self, frame: &mut Frame, area: Rect) {
        ...
        push_field(&mut lines, "Last", &b.last_changes, Color::LightGreen);
        push_field(&mut lines, "Next", &b.next_step, Color::LightMagenta);
        if !b.active_scope.trim().is_empty() {
            push_field(&mut lines, "Scope", &b.active_scope, Color::DarkGray);
        }
    ...
    pub(crate) fn draw_folder_notes_panel(&self, frame: &mut Frame, area: Rect) {
        ...
        if self.folder_notes.is_empty() {
            ...
            for (index, note) in self.folder_notes.iter().enumerate().skip(start).take(4) {
                ...
                lines.push(Line::from(vec![
                    Span::raw("  next: "),
                    Span::styled(note.next_step.as_str(), Style::default().fg(Color::DarkGray)),
                ]));
                lines.push(Line::from(vec![
                    Span::raw("  arch: "),
        ...
        if self.tool_timeline.is_empty() {
            ...
            for entry in self.tool_timeline.iter().rev().take(8).rev() {
                ...
                lines.push(Line::from(vec![
                    ...
                    Span::styled(entry.name.as_str(), Style::default().fg(Color::LightMagenta)),
                ]));
                lines.push(Line::from(vec![
                    Span::raw("  "),
                    Span::styled(entry.summary.as_str(), Style::default().fg(Color::DarkGray)),
                ]));
            }
        }

        let paragraph = Paragraph::new(Text::from(lines))
            .block(panel_block(" Tools ", Color::LightGreen, Color::LightMagenta))
            .wrap(Wrap { trim: false });
    ...
    pub(crate) fn draw_scope_lock_panel(&self, frame: &mut Frame, area: Rect) {
        ...
        push_inline(&mut lines, "Launch", &s.launch_scope, Color::DarkGray);
        ...
        if !s.warning.trim().is_empty() {
            push_inline(&mut lines, "Check", &s.warning, Color::LightMagenta);
        }
